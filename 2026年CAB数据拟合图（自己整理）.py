...
# 数据预处理：强制转换为数值并清洗空值 (新增对'9波段'列的处理)
df['cab'] = pd.to_numeric(df['cab'], errors='coerce')
df['6波段'] = pd.to_numeric(df['6波段'], errors='coerce')
df['LICI指数算法'] = pd.to_numeric(df['LICI指数算法'], errors='coerce')
df['9波段'] = pd.to_numeric(df['9波段'], errors='coerce')

# 清洗同时包含实测值和三种算法预测值的有效行
df_clean = df.dropna(subset=['cab', '6波段', 'LICI指数算法', '9波段'])

# 提取变量：x 为实测值，y 为设备的预测值
x = df_clean['cab']
y1 = df_clean['6波段']
y2 = df_clean['LICI指数算法']
y3 = df_clean['9波段']  # 新增的9波段预测值
...
plt.plot(x_fit, slope2 * x_fit + intercept2, color='#d62728', linestyle='--', linewidth=2,
         label=f'LICI 指数算法 ($R^2={r2_2:.3f}$, $RMSE={rmse_2:.3f}$)')
...
# 让 1:1 线稍微延伸出数据范围一点，更好看
plt.plot([min_val-5, max_val+5], [min_val-5, max_val+5], color='gray', linestyle=':', linewidth=1.5, label='1:1 Line')

# 3. 图表细节美化
# ==========================================================
# 核心修改点：强制设定 X 和 Y 轴的显示范围
# 建议您根据 cab 的实际范围调整这两个数字，比如 30 到 80
plt.xlim(30, 70)
plt.ylim(30, 70)
# 如果您想设置刻度密度，还可以加上：
# plt.xticks(np.arange(35, 71, 5))
# plt.yticks(np.arange(35, 71, 5))
# ==========================================================

plt.xlabel('Measured $C_{ab}$ ($\mu g/cm^2$)', fontsize=12, fontweight='bold')
plt.ylabel('Predicted $C_{ab}$ ($\mu g/cm^2$)', fontsize=12, fontweight='bold')
plt.title('Algorithm Fitting Comparison for Chlorophyll Content', fontsize=14, fontweight='bold')
...
# 4. 保存与展示
plt.tight_layout()
plt.savefig('fitting_plot_3_Algos.png', dpi=300)
plt.show()