...
# 🌟 2. 核心联动：上一步训练好的 27 参模型与 Scaler 所在目录
MODEL_DIR = r"E:\叶绿素反演-李文娟老师论文\新采用的9波段\反演Cab-ANN-model\核参数一起算\weiss参数\27核参数"

# 🌟 3. 核心联动：第一步 6S 生成的 9 波段多项式系数 CSV
COEFFS_CSV_FILE = r"E:\叶绿素反演-李文娟老师论文\新采用的9波段\基于6s的公式系数\9_bands_coefficients.csv"

# 4. 输出结果路径配置 (更新为 27 参专用输出目录)
OUTPUT_DIR = r"E:\叶绿素反演-李文娟老师论文\新采用的9波段\反演-Result"
...
# 反演时间范围控制 (支持精确到具体生育期)
START_DATE = "2026-03-01"  # 起始日期 (格式: YYYY-MM-DD)
END_DATE = "2026-12-02"  # 结束日期 (格式: YYYY-MM-DD)

# 🌟 6. 动态加载 9 个目标波段与多项式系数
...
print(f"✅ 成功对接 6S 系数，波段: {TARGET_BANDS}")
...
def main():
    print("🚀 开始实测数据反演 (包含 27 核参数滑动窗口优化与高级图表输出)...")

    # 1. 加载与过滤数据
    if not os.path.exists(REAL_DATA_PATH):
        print(f"❌ 找不到实测数据文件: {REAL_DATA_PATH}")
        return
    ...
    # 漫射比例估算 (沿用第一步训练的模型估算真实场景下的总 PAR 散射比)
    step1_model_path = os.path.join(MODEL_DIR, "bp_ann_df_model.pkl")  # 如果没在这个文件夹，请修改路径或取消这部分依赖
    ...
    for current_date in unique_dates:
        ...
        # 保证观测数至少大于未知数数量 (27) 以保证最小二乘解的可靠性
        ...
        if res.success:
            ...
            day_coeffs['mean_fit_rmse'] = np.sqrt(res.fun / (n_bands * len(window_df)))
            results.append(day_coeffs)

    res_df = pd.DataFrame(results)
    if res_df.empty:
        print("❌ 拟合失败，未能提取出有效核参数。可能是有效天数观测不满足滑动窗口最小采样点要求。")
        return
    ...
    # 6. ANN 预测
    print("🧠 正在使用集成 BP 神经网络预测生理参数 (27核参数输入)...")
    scaler_path = os.path.join(MODEL_DIR, "scaler_inversion.pkl")
    if not os.path.exists(scaler_path):
        print(f"❌ 找不到 Scaler 文件: {scaler_path}，无法进行预测！")
        return
    ...
    if len(feature_cols) != n_bands * 3:
        print(f"❌ 提取的核特征数量不等于 {n_bands * 3}，无法对接 ANN！请检查。")
        return
    ...
    res_df.to_csv(os.path.join(OUTPUT_DIR, f"Final_Inversion_Results_{TARGET_YEAR}_27Params.csv"), index=False)
    ...
    # =========================================================================
    # 8. 图表一：按核参数分类时间序列（3张子图，每张展示9个波段）
    # =========================================================================
    ...
    # 为 9 个波段分配不同的颜色 (可以使用 colormap 自动取色)
    ...
    # =========================================================================
    # 9. 🌟 升级优化：图表二：按波段分类时间序列 (3 x 3 布局，共9波段)
    # =========================================================================
    ...
    print(
        f"👉 本次输出包含:\n 1. Final_Inversion_Results_{TARGET_YEAR}_27Params.csv (实测反演数值结果)\n 2. Cab 预测时序图\n 3. 核参数(按类型)时序面板图(3子图)\n 4. 核参数(按9个波段)时序面板图(3x3=9子图)")
