...
# 输出路径 (最终的综合训练集)
SAVE_PATH = r"E:\叶绿素反演-李文娟老师论文\新采用的9波段\Prosail-data\soil-select\weiss参数设置\特征约束\18核参数一起算\prosail_training_database_hybrid.csv"

# 原始 PROSAIL 光谱参数文件
PROSAIL_CSV_FILE = r"E:\叶绿素反演-李文娟老师论文\prosail配置文件\dataSpec_P5.csv"

# 🌟 本地化机理：引入三条南京白马基地实测土壤光谱库
CUSTOM_SOIL_FILES = [
    r"E:\叶绿素反演-李文娟老师论文\白马土壤数据\24年小麦播种前裸土\PROSAIL_Trimmed_Soil_Background.csv",  # 50% 小麦旱地
    r"E:\叶绿素反演-李文娟老师论文\白马土壤数据\25年水稻种植前泡田\PROSAIL_Trimmed_Soil_Background.csv",  # 35% 水稻早期水体
    r"E:\叶绿素反演-李文娟老师论文\白马土壤数据\25年水稻播种前裸土\PROSAIL_Trimmed_Soil_Background.csv",  # 15% 水稻中后期湿泥
]
...
IOTA_BANDS = [610, 680, 730, 760, 810, 860]
BAND_NAMES = [f"{b}nm" for b in IOTA_BANDS]
# 确保波段索引对齐 440nm
BAND_INDICES = [w - 440 for w in IOTA_BANDS]
        ...
        if custom_soil_paths:
            for path in custom_soil_paths:
                if os.path.exists(path):
                    ...
                    custom_soil = df_soil.iloc[:, col_idx].values
...
def get_weiss_parameters(n_samples):
    print("🎲 正在使用拉丁超立方 (LHS) 进行高密度参数空间采样 (已放宽物理下限)...")
    ...
    min_u_lai_late, max_u_lai_late = np.exp(-0.5 * 7), np.exp(-0.5 * 0.1)

    mapped_u_lai = np.where(soil_idx == 2, u_lai * (max_u_lai_late - min_u_lai_late) + min_u_lai_late,
                            u_lai * (max_u_lai_global - min_u_lai_global) + min_u_lai_global)
    lai = -2 * np.log(mapped_u_lai)

    # 🌟 修改：放宽 Cab 下限至 1.0，上限至 80.0
    ...
    # 🌟🌟🌟 新增：注入 3% 的纯背景(裸土/泡田)边界锚点数据 🌟🌟🌟
    print("🌱 正在注入 3% 的纯背景边界锚点数据 (LAI=0, Cab=0)...")
    np.random.seed(42)  # 保证每次生成的索引一样
    pure_bg_mask = np.random.rand(n_samples) < 0.02

    # 强行将这 3% 数据的冠层特征抹除，留下纯土壤/水体信号
...
def process_case(args):
    ...
    obs_abs_refls = np.zeros((6, T))
    f_lams_matrix = np.zeros((6, T))
    for i, band in enumerate(BAND_NAMES):
    ...
    k_vol_dlc_all = np.zeros((6, T))
    k_geo_dlc_all = np.zeros((6, T))
    for i in range(6):
...
if __name__ == "__main__":
    print(f"🚀 初始化 PROSAIL 核参数联合求解数据生成引擎 (N_CASES={N_CASES})...")

    daily_geometries = load_geometry_groups(GEOMETRY_FILE)
    if not daily_geometries:
        raise ValueError("❌ 未能从几何文件中加载到任何数据，请检查 CSV 文件！")
    ...
    print(f"⚡ 启动多进程计算 (18参数全局优化 + 尺度锚定)...")
    with Pool(cpu_count()) as p:
        results = [r for r in tqdm(p.imap(process_case, tasks), total=N_CASES) if r is not None]

    if not results:
        print("❌ 所有案例拟合均失败。请检查输入参数。")
    else:
        final_df = pd.DataFrame(results)
        os.makedirs(os.path.dirname(SAVE_PATH), exist_ok=True)
        final_df.to_csv(SAVE_PATH, index=False)
        print(f"✅ 完成! 最终有效样本数: {len(final_df)}，已保存至: {SAVE_PATH}")
        print(f"🔥 数据集已准备就绪，可以输入 BP-ANN 进行训练了！")